...
## 🕐 13:30 – 14:00 說明段(30 分鐘)

| 時間 | 主題 | 重點 |
| :--- | :--- | :--- |
| 13:30 – 13:35 | **開場 + 成品 Demo** | 直接展示做完的 To-Do App,再展示 `/fix-issue` 自動開 PR 的畫面。**先給結果,再講原理** |
| 13:35 – 13:45 | **Agent Mode** | Ask / Edit / Agent 對照表。核心一句話:**你給的是目標,不是步驟** |
| 13:45 – 13:52 | **MCP** | 「AI 的 USB-C」。強調安全:只裝信任來源、授權對話框要看 |
| 13:52 – 13:58 | **Agentic Workflow** | 公式:**規矩 + 劇本 + 手腳**。強調「可版控、可 review、團隊共用」 |
| 13:58 – 14:00 | **環境檢查 + 建 repo** | 帶大家一起按 **Use this template**,順便抓出環境有問題的人 |
...
| 掛鐘時間 | 應該在哪 | 落後的話 |
| :--- | :--- | :--- |
| **14:15** | 大家 repo 建好、clone 好、issue 出現 | 環境有問題的人先兩人一組 |
| **14:45** | Step 1 完成(To-Do App 跑起來) | **這關最重要,寧可多花 5 分鐘** |
| **15:05** | Step 2 完成 | 落後就把「四種還原方法」改成只講師示範,學員只做方法一 |
| **15:35** | Step 3 完成(MCP 接上) | **這關也是重點**。GitHub MCP 接不上就只做 Microsoft Learn |
| **15:55** | Step 4 完成 | 落後就直接用 `solutions/step-4/` 複製,講師示範 `/fix-issue` 執行過程 |
| **16:00** | 測驗 + 收尾 | 測驗可以帶著念,或請學員回家自己做 |

### ⏰ 三個必須喊停的時間點

- **14:45** — 不管 Step 1 有沒有做完,**喊停,統一用 solutions 補齊**,然後往下走
- **15:35** — Step 3 一定要在這時結束,否則 Step 4 會做不完
- **15:55** — 強制收尾,留 5 分鐘做測驗與總結
